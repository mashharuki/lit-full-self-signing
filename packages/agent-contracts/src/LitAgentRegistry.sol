...
contract LitAgentRegistry {
    ...
    event ActionPolicySet(address indexed user, address indexed pkp, string ipfsCid, bytes description, bytes policy);

    /// @dev Emitted when an action policy is removed
    event ActionPolicyRemoved(address indexed user, address indexed pkp, string ipfsCid);
     ...
     */
    function setActionPolicy(address pkp, string calldata ipfsCid, bytes calldata description, bytes calldata policy)
        external
    {
    ...
    function removeActionPolicy(address pkp, string calldata ipfsCid) external {
        ...
        for (uint256 i = 0; i < config.registeredActions.length; i++) {
     ...
     */
    function getActionPolicy(address user, address pkp, string calldata ipfsCid)
        external
        view
        returns (bool isPermitted, bytes memory description, bytes memory policy)
    {
    ...
    function getRegisteredActions(address user, address pkp)
        external
        view
        returns (string[] memory ipfsCids, bytes[] memory descriptions, bytes[] memory policies)
    {
        require(pkpConfigs[user][pkp].isRegistered, "PKP not registered");
